...
function sendItemUpdateRequest() {
  ...
  sendAjaxRequest(
      'post', '/api/item/' + id, {done: checked}, itemUpdatedHandler);
}
...
function sendCreateItemRequest(event) {
  ...
  if (description != '')
    sendAjaxRequest(
        'put', '/api/cards/' + id, {description: description},
        itemAddedHandler);
...
function itemAddedHandler() {
  ...
  // Insert the new item
  let card =
      document.querySelector('article.card[data-id="' + item.card_id + '"]');
...
function cardDeletedHandler() {
  ...
  let article =
      document.querySelector('article.card[data-id="' + card.id + '"]');
  article.remove();
}
